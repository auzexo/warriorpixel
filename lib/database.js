...
export const joinTournament = async (tournamentId, userId, inGameName, voucherType = null) => {
  try {
    ...
    // Update count using RPC function (prevents race conditions)
    const { data: countData, error: countError } = await supabase
      .rpc('increment_participant_count', { tournament_id: tournamentId });
    ...
    if (countError) {
      ...
      await supabase
        .from('tournaments')
        .update({ participants_count: (tournament.participants_count || 0) + 1 })
        .eq('id', tournamentId);
    }

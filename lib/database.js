...
export const getTournamentWithDetails = async (tournamentId, userId) => {
  try {
    ...
    // Check if room details should be visible (5 min before tournament)
    const now = new Date();
    const roomVisibleTime = tournament.room_visible_at ? new Date(tournament.room_visible_at) : null;
    const canSeeRoom = participation && roomVisibleTime && now >= roomVisibleTime;
...
export const joinTournament = async (tournamentId, userId, voucherType = null) => {
  try {
    // Get tournament
...
export const giveWinTag = async (userId, tournamentId, prizeAmount) => {
  try {
    ...
    // Update user wins and add prize
    await supabase
      .from('users')
      .update({

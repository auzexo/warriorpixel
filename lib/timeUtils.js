...
/**
 * Check if a tournament's start time has passed
 * Used by tournamentStatusUpdater.js
 */
 ...
 * Used in admin tournament create/edit forms
 */
export const parseISTToUTC = (istLocalString) => {
  if (!istLocalString) return null;
  // Append +05:30 to explicitly tell JS this is IST
  // This is the ONLY correct way — no manual math needed
  return new Date(istLocalString + ':00+05:30').toISOString();
};

/**
 * Format UTC timestamp → IST datetime-local input value
 * Used to pre-fill datetime-local inputs in IST
 */
export const formatUTCToISTLocal = (utcString) => {
  if (!utcString) return '';
  const istMs = new Date(utcString).getTime() + 19800000;
